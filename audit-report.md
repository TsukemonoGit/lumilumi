...
- `hooks.server.ts` にて全レスポンスにセキュリティヘッダーを一括付与
  ...
  - `frame-src` に YouTube・Twitter・Bluesky の埋め込み用ホストを許可（`https://www.youtube.com https://www.youtube-nocookie.com https://twitter.com https://platform.twitter.com https://bsky.app`）
  - `script-src` に `https://platform.twitter.com` を追加（`widgets.js` の読み込みおよび動的iframe生成に必要）
