...
### H-3. `markdown-it-link.ts` が `javascript:` スキームを許可
...
### H-4. `allowedOrigins` にトレーリングスラッシュでCORSが機能しない
...
### H-6. `filterNaddr` が `d` タグをインデックス0固定で探索 (NIP-01違反)
...
### H-7. `zappedPubkey` が NIP-57 に準拠していない
...
### H-8. `usePromiseReq` のエラーパスでsubscriptionリーク
...
### H-16. LocalStorage に全フォロイーのメタデータを同期的にJSON書き込み
...
### M-3. `validateServerConfiguration` の delegated_to_url 検証ロジック反転
...
### M-5. `latestEachNaddr` で tags 空配列時に TypeError
...
### M-6. `mute.ts` のタイムスタンプ比較が `>=` (同一イベントで上書き)
...
### M-11. `muteCheck` が1回の呼び出しで `get(mutes)` を最大5回
...
### M-12. `Content.svelte` が `ContentParts` を常時2重インスタンス化
...
### M-15. `[npub=npub]/+page.svelte` が770行 — Metadata+EventCard 5回重複
